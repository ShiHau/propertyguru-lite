...
def admin_dashboard(request: Request):
    ...
    return templates.TemplateResponse(
        request, "admin/portal_admin_dashboard.html", context
    )


@router.get(
    "/portal/admin/agents", response_class=HTMLResponse, include_in_schema=False
)
def admin_agents_list(request: Request):
    ...
    try:
        ...
        if not context["error_message"]:
            context["error_message"] = str(exc)
    return templates.TemplateResponse(
        request, "admin/portal_admin_agents.html", context
    )


@router.get(
    "/portal/admin/agents/{user_id}",
    ...
    try:
        ...
        context["agent"] = next(
            (
                user
                for user in users
                if user.get("role") == "agent" and user.get("id") == user_id
            ),
            None,
        )
        ...
        if not context["error_message"]:
            context["error_message"] = str(exc)
    return templates.TemplateResponse(
        request, "admin/portal_admin_agent_detail.html", context
    )
...
def admin_update_agent(
    ...
    if not payload:
        return redirect_with_message(
            f"/portal/admin/agents/{user_id}", error="No update values provided"
        )

    try:
        admin.update_user(token, user_id=user_id, role="agent", payload=payload)
        return redirect_with_message(
            f"/portal/admin/agents/{user_id}", success="Agent updated"
        )
    except common.BackendClientError as exc:
...
def admin_toggle_agent_active(request: Request, user_id: int):
    ...
    try:
        ...
        if not agent:
            return redirect_with_message(
                f"/portal/admin/agents/{user_id}", error="Agent not found"
            )

        new_state = not bool(agent.get("is_active"))
        admin.update_user(
            token, user_id=user_id, role="agent", payload={"is_active": new_state}
        )
        status_text = "active" if new_state else "inactive"
        return redirect_with_message(
            f"/portal/admin/agents/{user_id}", success=f"Agent set to {status_text}"
        )
    except common.BackendClientError as exc:
        return redirect_with_message(f"/portal/admin/agents/{user_id}", error=str(exc))
...
def admin_deactivate_agent(request: Request, user_id: int):
    ...
    try:
        ...
        return redirect_with_message(
            f"/portal/admin/agents/{user_id}", success="Agent deactivated"
        )
    except common.BackendClientError as exc:
        return redirect_with_message(f"/portal/admin/agents/{user_id}", error=str(exc))
...
@router.get(
    "/portal/admin/inquiries", response_class=HTMLResponse, include_in_schema=False
)
def admin_inquiries_list(request: Request):
    ...
    context = base_context(request)
    context.update(
        {"error_message": request.query_params.get("error"), "inquiries": []}
    )
    try:
        ...
        if not context["error_message"]:
            context["error_message"] = str(exc)
    return templates.TemplateResponse(
        request, "admin/portal_admin_inquiries.html", context
    )


@router.get(
    "/portal/admin/inquiries/{inquiry_id}",
    response_class=HTMLResponse,
    include_in_schema=False,
)
def admin_inquiry_detail(request: Request, inquiry_id: int):
    ...
    context = base_context(request)
    context.update(
        {"error_message": request.query_params.get("error"), "inquiry": None}
    )
    try:
        ...
        if isinstance(listing_id, int):
            try:
                listings = admin.get_listings(token, limit=500)
                listing = next(
                    (item for item in listings if item.get("id") == listing_id), None
                )
            ...
            try:
                ...
                agent = next(
                    (
                        item
                        for item in users
                        if item.get("id") == agent_id and item.get("role") == "agent"
                    ),
                    None,
                )
        ...
        if not context["error_message"]:
            context["error_message"] = str(exc)
    return templates.TemplateResponse(
        request, "admin/portal_admin_inquiry_detail.html", context
    )


@router.get(
    "/portal/admin/listings", response_class=HTMLResponse, include_in_schema=False
)
def admin_listings_list(request: Request):
    ...
    try:
        ...
        if not context["error_message"]:
            context["error_message"] = str(exc)
    return templates.TemplateResponse(
        request, "admin/portal_admin_listings.html", context
    )


@router.get(
    "/portal/admin/listings/new", response_class=HTMLResponse, include_in_schema=False
)
def admin_listing_create_form(request: Request):
    ...
    return templates.TemplateResponse(
        request, "admin/portal_admin_listing_new.html", context
    )
...
def admin_listing_create(
    ...
    try:
        ...
        return redirect_with_message(
            "/portal/admin/listings", success="Listing created"
        )
...
@router.get(
    "/portal/admin/listings/{listing_id}",
    response_class=HTMLResponse,
    include_in_schema=False,
)
def admin_listing_detail(request: Request, listing_id: int):
    ...
    try:
        ...
        context["listing"] = next(
            (listing for listing in listings if listing.get("id") == listing_id), None
        )
        ...
        if not context["error_message"]:
            context["error_message"] = str(exc)
    return templates.TemplateResponse(
        request, "admin/portal_admin_listing_detail.html", context
    )
...
def admin_listing_update(
    ...
    if not payload:
        return redirect_with_message(
            f"/portal/admin/listings/{listing_id}", error="No update values provided"
        )

    try:
        admin.update_listing(token, listing_id, payload)
        return redirect_with_message(
            f"/portal/admin/listings/{listing_id}", success="Listing updated"
        )
    except (common.BackendClientError, ValueError) as exc:
        return redirect_with_message(
            f"/portal/admin/listings/{listing_id}", error=str(exc)
        )
